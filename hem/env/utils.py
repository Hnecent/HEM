...
class Renderer:
    ...
    def _episode(self):
        ...
        for k, v in self._cal_kpis().items():
            if k == 'AC_control' or k == 'BESS_control' or k == 'washer_control' or k == 'BESS_electrical_consumption_contribution' or k == 'BESS_electrical_cost_contribution':
                print(f'{k}: {v}')
            else:
                print(f'{k}: {v:.2f}')
    ...
    def _cal_kpis(self):
        ...
        electrical_consumption_sum = CostFunction.electrical_consumption(
            self.state['home_electrical_consumption_history']) - self.state['BESS_energy_history'][-1]

        electrical_cost_sum = CostFunction.electrical_cost(self.state['home_electrical_cost_history']) - \
                              self.state['BESS_energy_history'][-1] * 0.05113 * 100
        ...
        # carbon_production_sum = sum(self.state['carbon_production_history']) - self.state[
        #     'BESS_energy_history'][-1] * 0.17513
        carbon_production_sum = sum(self.state['carbon_production_history'])
        ...
        return {
            ...
            'BESS_electrical_consumption_contribution': f'{electrical_consumption_sum:.2f} / {no_BESS_electrical_consumption_sum:.2f}, {BESS_electrical_consumption_contribution:.2f}',
            'BESS_electrical_cost_contribution': f'{electrical_cost_sum:.2f} / {no_BESS_electrical_cost_sum:.2f}, {BESS_electrical_cost_contribution:.2f}',
            # 'BESS_carbon_production_contribution': BESS_carbon_production_contribution
            'AC_control': f'{AC_control} / {len(self.state["AC_control_history"])}, {AC_control / len(self.state["AC_control_history"]):.2f}',
            'BESS_control': f'{BESS_control} / {len(self.state["BESS_control_history"])}, {BESS_control / len(self.state["BESS_control_history"]):.2f}',
            'washer_control': f'{washer_control} / {len(self.state["washer_control_history"])}, {washer_control / len(self.state["washer_control_history"]):.2f}',
            'AC_reward': AC_reward,
