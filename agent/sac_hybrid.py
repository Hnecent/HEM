...
class SAC:
    ...
    def __init__(self, state_dim, net_arch, action_dim, action_bound,
                 ...
                 device):
        self.actor = PolicyNet(state_dim, action_dim, net_arch, action_bound).to(device)  # 策略网络
        self.critic_1 = QValueNet(state_dim, action_dim, net_arch).to(device)  # 第一个Q网络
        self.critic_2 = QValueNet(state_dim, action_dim, net_arch).to(device)  # 第二个Q网络
        self.target_critic_1 = QValueNet(state_dim, action_dim, net_arch).to(
            device)  # 第一个目标Q网络
        self.target_critic_2 = QValueNet(state_dim, action_dim, net_arch).to(
            device)  # 第二个目标Q网络
        # 令目标Q网络的初始参数和Q网络一样
